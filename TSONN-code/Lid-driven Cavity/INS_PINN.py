...
class PINN():
    ...
    def Msef(self):
        X = self.X
        pred = self.model(X)
        u = pred[:,[0]];  v = pred[:,[1]];   p = pred[:,[2]]; 
              
        u_xy = fwd_gradients(u, X)
        v_xy = fwd_gradients(v, X)
        p_xy = fwd_gradients(p, X)
        u_x = u_xy[:,[0]]; u_y = u_xy[:,[1]]
        v_x = v_xy[:,[0]]; v_y = v_xy[:,[1]]
        p_x = p_xy[:,[0]]; p_y = p_xy[:,[1]]
        
        u_xx = fwd_gradients(u_x, X)[:,[0]]
        u_yy = fwd_gradients(u_y, X)[:,[1]]
        v_xx = fwd_gradients(v_x, X)[:,[0]]
        v_yy = fwd_gradients(v_y, X)[:,[1]]
